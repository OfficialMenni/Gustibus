import unittest

from django.contrib.auth.models import User
from django.test import TestCase
from django.test.client import Client
from django.urls import resolve
from django.urls import reverse

from shoppingcart.views import *
from users.models import Vendor


class TestUrls(unittest.TestCase):

    def test_order_summary_url(self):
        path = reverse('order-summary')
        self.assertEquals(resolve(path).func, order_details)

    def test_order_list_url(self):
        path = reverse('order-list')
        self.assertEquals(resolve(path).func.view_class, OrdersListView)

    def test_add_to_cart_url(self):
        path = reverse('add-to-cart', kwargs={'item_id': 1})
        self.assertEquals(resolve(path).func, add_to_cart)

    def test_update_transaction_record_url(self):
        path = reverse("update-records", kwargs={"order_id": 1})
        self.assertEquals(resolve(path).func, update_transaction_records)

    def test_remove_from_cart_url(self):
        path = reverse('remove-item', kwargs={'item_id': 1})
        self.assertEquals(resolve(path).func, remove_from_cart)


class ViewsTest(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(username="resu", password="TESTING321")
        self.user_vendor = User.objects.create_user(username="vendoruser", password="TESTING321")
        self.profile = Profile.objects.get(user=self.user)
        self.user_logged = Client()
        self.vendor_logged = Client()
        self.vendor_logged.login(username="vendoruser", password="TESTING321")
        self.user_logged.force_login(self.user)
        self.admin_user = User.objects.create_superuser(username="Super", password="TESTING321")
        self.admin_user_logged = Client()
        self.admin_user_logged.force_login(self.admin_user)
        self.vendor = Vendor.objects.create(user=self.user_vendor, address="indirizzo", mobile="123456789",
                                            description="ABCDEFGH", ranknum=0, ranksum=0)
        self.product = Product.objects.create(item="test", description="testd", category="Desserts",
                                              cost=10, available=True,
                                              vendor=self.vendor)
        self.order_item = OrderItem.objects.get_or_create(product=self.product, quantity=10)
        self.user_order = Order.objects.get_or_create(owner=self.profile, is_ordered=False)
        self.user_order_true = Order.objects.get_or_create(owner=self.profile, is_ordered=True)

    def test_AddToCart_post(self):
        response = self.user_logged.post((reverse("add-to-cart", kwargs={"item_id": 1})), data={"quantity": 2})
        self.assertRedirects(response, '/', status_code=302,
                             target_status_code=200, fetch_redirect_response=False)

    def test_RemoveFromCart_post(self):
        response = self.user_logged.post((reverse("remove-item", kwargs={"item_id": 1})))
        self.assertRedirects(response, '/shoppingcart/order-summary', status_code=302,
                             target_status_code=200, fetch_redirect_response=False)

    def test_OrderDetails_get(self):
        response = self.user_logged.get((reverse("order-summary")))
        self.assertEquals(response.status_code, 200)

    def test_UpdateTransaction_post(self):
        response = self.user_logged.post((reverse("update-records", kwargs={"order_id": 1})))
        self.assertRedirects(response, '/', status_code=302,
                             target_status_code=200, fetch_redirect_response=False)

    def test_OrderList_get(self):
        response = self.user_logged.get((reverse("order-list")))
        self.assertEquals(response.status_code, 200)
